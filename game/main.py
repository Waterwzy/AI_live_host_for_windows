...
def init_board():
    ...
    for i in range(1,11):
        text_for_x.append(font.render(chr(i+ord('A')-1),True,(0,0,0)))
        text_for_y.append(font.render(str(i), True, (0,0,0)))
...
image_player=pygame.image.load('game\\player.png')
image_firefly=pygame.image.load('game\\firefly.png')
processlist=0
game_list=[]
user_name=''
finish_time=0
...
while not done:
    ...
    game_list=read_command()

    if game_list != None :


        if processlist <= len(game_list)-1 and not finish:
            print('get cmd!')
            if game_list[processlist]['cmd'] == 'down':
                #print('down!')
                #print(game_list[processlist]['message'])
                move=downboard((40+game_list[processlist]['message'][0]*50,40+game_list[processlist]['message'][1]*50))
            elif game_list[processlist]['cmd'] == 'start':
                user_name=game_list[processlist]['message']
            processlist+=1
    if move :
        ...
        #print("min:",nowmin)
        if nowmin== math.inf:
            textx=font.render(user_name+"胜利",True,(0,0,0))
    ...
    if finish_time and time.time()-finish_time > 3 :
        break
